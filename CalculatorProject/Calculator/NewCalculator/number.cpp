...
void Number::setCheckRadixWhichClicked(int radixCheck)
{
    m_radixCheck = radixCheck;
    qDebug("m_radixCheck: %d", m_radixCheck);
}
void Number::setNumberInput(int numberInput)
{
    //m_numberInput = to_string(numberInput);
    //if(m_radixCheck == 1) castToDecimal(HEX_RADIX_TYPE);
    //else if(m_radixCheck == 2) castToDecimal(DEC_RADIX_TYPE);
    //else if(m_radixCheck == 3) castToDecimal(OCT_RADIX_TYPE);
    //else if(m_radixCheck == 4) castToDecimal(BIN_RADIX_TYPE);
    //castToDecimal(DEC_RADIX_TYPE);
    m_number = numberInput;
    setNumberFromDecToRadixOther();
}

void Number::castToDecimal(RADIX_TYPE radixType)// unuse
{
    switch (radixType) {
    case HEX_RADIX_TYPE:
...
void Number::setNumberFromDecToRadixOther()
{
    ...
    qDebug("Number, m_decNumber: %d", m_decNumber);
    m_hexNumber = castFromDecToHex(m_decNumber);
    m_octNumber = castFromDecToOct(m_decNumber);
    m_binNumber = castFromDecToBin(m_decNumber);

    qDebug() << "Number, m_hexNumber: " << m_hexNumber;
    qDebug() << "Number, m_decNumber: " << m_decNumber;
    qDebug() << "Number, m_octNumber: " << m_octNumber;
    qDebug() << "Number, m_binNumber: " << m_binNumber;
